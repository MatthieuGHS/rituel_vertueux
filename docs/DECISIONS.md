...
| 2 | Détourés | Sticks détourés depuis les visuels studio HD (#00, #01). Boîtes (v0.1) : PNG fournis par la cliente (`pngcorrectR/J.png`), finalisés par `finish_box_cutout` : enveloppe convexe calculée sur les seuls pixels de carton (l’ombre au sol restée opaque est retirée), bords rétractés d’1 px, liseré blanc recoloré sans toucher aux arêtes sombres. | Les premiers PNG fournis (`IMG_3426*`) étaient trop petits ; les détourages automatiques depuis la photo gardaient une partie de l’ombre. |
